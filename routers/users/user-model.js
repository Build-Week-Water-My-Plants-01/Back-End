...
function getPlants(id) {
	return db('plants as p')
		...
		.select('p.id', 'p.nickname', 'p.h2o_frequency as frequency', 'p.image', 'p.species_name')
		.where({ 'p.user_id': id });
}

function getUserById(id) {
	return db('users').select('username', 'phone_number').where({ id }).first();
}
...
function updatePlant(plantData, plantid) {
	return db('plants').where({ id: plantid }).update(plantData).then((success) => {
...
function getSpecies(filter) {
	return db('species').where(filter).first();
}

function addSpecies(species) {
	return db('species').insert(species, 'id').then((ids) => {
		const [ id ] = ids;
